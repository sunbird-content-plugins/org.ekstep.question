...
.controller('QuestionCreationFormController', ['$scope', 'instance', 'questionData', function ($scope, instance, questionData) {
  ...
  $scope.questionMetaData = {};
	...
	$scope.loadPlugins = function(plugins, manifestMedia, cb) {
		var pluginObj = [];
	...
	$scope.showTemplates = function() {
		$scope.templatesScreen = true;
		$scope.questionMetadataScreen = false;
		var qsManifest = org.ekstep.pluginframework.pluginManager.getPluginManifest($scope._constants.questionsetPlugin);
		...
		var data = {
			"request": {
				"filters": {
					"objectType": ["Content"],
					"contentType": ["Plugin"],
					"targets.id": $scope._constants.questionsetPlugin,
		...
		ecEditor.getService('search').search(data, function(err, resp) {
			...
			$scope.loadPlugins(plugins, [], function() {
